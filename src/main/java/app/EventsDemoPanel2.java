...
//https://www.youtube.com/watch?v=22MBsRYuM4Q
		...
		save.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//int n = JOptionPane.showOptionDialog(this, "Would you like to save the file as:  " , "Save as ", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
				DefaultTableModel model = (DefaultTableModel) table.getModel();
	...
	private void RestrictByDate(Set<DDSPurchases> detailsDTOs) {
		if(!period1Year.getSelectedItem().equals("----") && !period2Year.getSelectedItem().equals("----") && !period1Month.getSelectedItem().equals("--") && !period2Month.getSelectedItem().equals("--") && !period1Day.getSelectedItem().equals("--") && !period2Day.getSelectedItem().equals("--")) {
			for (DDSPurchases s : detailsDTOs) {
				java.util.Date begins = null;
				java.util.Date ends = null;
				DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
				String date1 = period1Year.getSelectedItem()+"-"+period1Month.getSelectedItem()+"-"+period1Day.getSelectedItem();
				try {
				begins = df.parse(date1);
				String date2 = period2Year.getSelectedItem()+"-"+period2Month.getSelectedItem()+"-"+period2Day.getSelectedItem();
					ends = df.parse(date2);
